...
# 
# Optimization using Pymoo with RVEA to tune HDBSCAN hyperparameters on top of text embeddings and the model text generation. The optimization objectives are the metrix BERTScore, RougeL and the UQ based on HBDSCAN-KNN.
# 
# Optimizes:
# 
# - HDBSCAN parameters
...
earlystop_smooth_k = 3 #do not judge early stopping using the raw hypervolume value from a single generation.
...
BASE_LOG_DIR = "/work/mamora/optimizacion/deep_seek_checkpoint/27DeepSeek_checkpoint_HDBSCAN_2OBJsv27_EarlyStop_RepSeed/"
...
def load_model_and_tokenizer_4bits(model_name):
    """
    Loads a pre-trained model and tokenizer with 4-bit quantization.
    :params:
    - model_name (str): The name or path of the pre-trained model from the Hugging Face Model Hub.

    :returns:
    - model: The loaded model with 4-bit quantization and adjustments for fine-tuning.
  ...
  """
    ...
    #In some cases it is not feasible to use it  
...
def main(seed: int, base_log_dir: str):
    ...
    # Variables    
    based_model_name = "mariamoracrossitcr/deepseek-llm-7b-base-MedQuadV1"
    model_name = ""    # adapter
    model_name_tokenizer = "mariamoracrossitcr/deepseek-llm-7b-base-MedQuadV1"
    answer_length = 200

    root_folder_path = "/data/mamora/medquad/"
    layer_embedding = "tscaling/"  #For backward compatibility
    ...
    #model, tokenizer = load_model_and_tokenizer_4bits(based_model_name)
    ...
    # Normalize answer andmodel_answer length to min length. Replace column model_answer and answer.
    #pred_trunc, ref_trunc = truncate_by_min_length(eval_df, pred_col="model_answer", ref_col="answer")
    
    #eval_df["model_answer"] = pred_trunc
    #eval_df["answer"] = ref_trunc
    ...
    Valore definitivos:
        ...
        earlystop_patience=6,  # Stops after n consecutive non improvements
        earlystop_rel_eps=2e-3, #Ignores tiny relative improvements. 
        earlystop_smooth_k=7,   # Uses last y gens average HV     
...
if __name__ == "__main__":
    "To send parameters from the OS" 
    import argparse
